...
const selectSuitableTest = (tests) => {
...
const getQuestionSnippet = (questionText) => {
  if (!questionText) return "";
  const snippet = String(questionText).split("___")[0].trim();
  return snippet || String(questionText).trim();
};
...
const answerQuestion = (question, index) => {
  ...
  if (question.type === "matching") {
    const leftColumn = Array.isArray(question.left_column)
...
describe("Test result flow", () => {
  it("answers correctly and shows the result", () => {
    ...
    cy.request("GET", "/api/tests").then((response) => {
      const selected = selectSuitableTest(response.body) || mockTest;
