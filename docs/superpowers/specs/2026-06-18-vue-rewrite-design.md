...
- **Этап 2 — Простые и публичные страницы.** Login, About, Contacts, Criteria, 404.
  Login — полноэкранный «голый» layout (без Header/Menu/Footer, через `meta.layout`).
  Здесь же базовая инфраструктура UI: уведомления-тосты (порт `showNotification`),
  валидация форм, компонент `SuccessModal`. Обкатка переноса вёрстки и CSS 1-в-1.
  > **Home перенесён в Этап 4**: его содержимое — каталог тем (Topics) с модалками, что
  > по сути материал Topics. До Этапа 4 Home остаётся простым лендингом-заглушкой.
...
- **Этап 4 — Темы (Topics) + Home.** Каталог тем на главной (Home), список тем, страница
  темы, рендер контента CKEditor (просмотр), модалки (вопросы/проект/тест).
...
**Принцип «тестов по объёму логики»** (применяется на всех этапах 2–6):
- **Логика → юнит-тесты (Vitest, по возможности TDD):** валидация форм, подсчёт баллов,
  типы вопросов (matching/ordering), guards, api-слой. Самая ценная страховка — пишем всегда.
- **Компоненты с поведением → компонентные тесты (@vue/test-utils):** формы (логин, профиль),
  навигация по вопросам, интерактив — рендер + действия пользователя + проверка вывода.
- **Чисто презентационные страницы (About, Contacts, Criteria) → лёгкий smoke** (монтируется
  без ошибок) + визуальная сверка; не покрываем детально.
- Чем больше логики в куске — тем больше тестов; презентацию покрываем тонко.

**Сетка безопасности:**
- Cypress e2e — основной инструмент сверки поведения; на Этапе 7 адаптируем существующие
...
- `vite build`, `type-check`, `lint`, `test:unit` должны проходить без ошибок в конце каждого этапа.
